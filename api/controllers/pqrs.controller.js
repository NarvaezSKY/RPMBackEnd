...
export const deletePQRS = async (req, res) => {
  try {
    ...
    if (!foundPQRS) return res.status(404).json({ message: "PQRS not found" });
...
export const updatePQRS = async (req, res) => {
  try {
    ...
    const PQRSUpdate = await PQRS.findByIdAndUpdate(id, newStatus, {
      new: true,
      runValidators: true,
    });
    if (!PQRSUpdate) {
    ...
    return res.status(200).json({ PQRSUpdate });
  } catch (error) {
    console.error(error);
